...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/xxf185/x-ui/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
————————————————
  ${green}15.${plain} 一键安装bbr
  ${green}16.${plain} 一键申请SSL证书(acme申请)
  ${green}17.${plain} SSL证书管理
 "
    show_status
